...
class TestUpright(unittest.TestCase):
    ...
    def test_upright(self):
        for specimen_id, specimen_name in self.id_to_specimen_name.items():
            ...
            raw_swc = str(files('skeleton_keys') / "example_data/{}".format(specimen_name))
            layer_drawings_file = files('skeleton_keys') / "example_data/{}_surfaces_and_layers.json".format(specimen_id)

            temp_output = str(files('skeleton_keys') / "example_data/{}_PytestUpright.swc".format(specimen_id))
            upright_args = {
            ...
            args_input = "".join([f" --{k} {v} " for k,v in upright_args.items()])
            upright_command = f"skelekeys-upright-corrected-swc {args_input}" 
            command_list = upright_command.split()
            subprocess.run(command_list, check=True)
            
            precalculate_swc = str(files('skeleton_keys') / "example_data/upright_swcs/{}.swc".format(specimen_id))
